...
# docker build -t veil-web .
# docker run -d -p 80:80 -e RAILS_MASTER_KEY=<value from config/master.key> --name veil-web veil-web
...
RUN apt-get update -qq && \
    apt-get install --no-install-recommends -y curl libjemalloc2 libvips postgresql-client imagemagick fonts-dejavu-core && \
